...
/** Fallback labels when a Sanity course row has no title (legacy IDs 1–5). */
...
/**
 * Courses are ordered like the public listing: `publishedAt desc`, 1-based ID = index + 1.
 */
export async function fetchCourseByListId(courseId: number): Promise<{ title: string } | null> {
  if (!Number.isFinite(courseId) || courseId < 1) return null;
  const row = await client.fetch<{ title?: string } | null>(
    `*[_type == "course"] | order(publishedAt desc) [$courseIndex]{ title }`,
