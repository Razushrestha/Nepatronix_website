...
export default function ContactClient({ initialData }: { initialData: ContactPageData }) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Check Template ID in EmailJS Dashboard";
      alert(`Error: ${message}`);
    }
  };
    ...
    <div className="bg-[#F8FAFC]">
      <section className="bg-[#020617] pt-44 pb-20 relative overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 text-center">
          ...
          <p className="mt-6 text-lg text-gray-300 sm:text-xl max-w-2xl mx-auto leading-relaxed">{data.pageDescription}</p>
        </div>
      </section>
        ...
        <div className="mx-auto grid max-w-6xl gap-6 px-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div key={item.label} className="rounded-2xl border border-slate-100 bg-white px-6 py-6 shadow-xl shadow-slate-200/50 flex flex-col items-center text-center gap-4 hover:-translate-y-1 transition-transform duration-300">
              <div className="flex justify-center items-center p-3 bg-blue-50 text-[#020617] rounded-xl">
                {item.icon}
              </div>
              <div>
              ...
              <form onSubmit={handleSubmit} className="mt-8 space-y-6">
                <div className="grid gap-6 sm:grid-cols-2">
                  <div>
                    ...
                    <input type="text" name="name" id="name" value={formData.name} onChange={handleInputChange} className="mt-2 block w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-[#020617] focus:border-[#C1121F] focus:bg-white focus:ring-2 focus:ring-[#C1121F]/20 transition-all outline-none" placeholder="Your full name" required />
                  </div>
                  <div>
                    <label htmlFor="email" className="block text-sm font-bold text-[#020617]">Email</label>
                    <input type="email" name="email" id="email" value={formData.email} onChange={handleInputChange} className="mt-2 block w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-[#020617] focus:border-[#C1121F] focus:bg-white focus:ring-2 focus:ring-[#C1121F]/20 transition-all outline-none" placeholder="you@example.com" required />
                  </div>
                </div>

                <div>
                  <label htmlFor="phone" className="block text-sm font-bold text-[#020617]">Phone number</label>
                  <input type="tel" name="phone" id="phone" value={formData.phone} onChange={handleInputChange} className="mt-2 block w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-[#020617] focus:border-[#C1121F] focus:bg-white focus:ring-2 focus:ring-[#C1121F]/20 transition-all outline-none" placeholder="+977 (984) 000-0000" />
                </div>

                <div>
                  <label htmlFor="message" className="block text-sm font-bold text-[#020617]">Message</label>
                  <textarea name="message" id="message" rows={4} value={formData.message} onChange={handleInputChange} className="mt-2 block w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-[#020617] focus:border-[#C1121F] focus:bg-white focus:ring-2 focus:ring-[#C1121F]/20 transition-all outline-none relative z-30 cursor-text" placeholder="How can we help you?" required />
                </div>

                <div className="pt-2">
                  <button type="submit" className="w-full rounded-xl bg-[#C1121F] px-8 py-4 text-base font-bold text-white shadow-lg shadow-red-900/20 transition-all hover:bg-[#A30F19] hover:-translate-y-1 active:scale-95 sm:w-auto">
                    Send Message
                  </button>
                </div>
              </form>
              ...
              <div className="rounded-3xl border border-slate-100 bg-white p-8">
                ...
                <div className="flex flex-wrap gap-4">
                  {socialLinks.map((social, i) => (
                    <a key={i} href={social.url} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-[#C1121F]/20 hover:bg-[#C1121F]/5 transition-all group">
