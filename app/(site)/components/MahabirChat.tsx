...
export default function MahabirChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "bot", text: "Namaste! I am Mahabir, your Nepatronix assistant. I know our courses, services, certificates, and contact details from the live website. Please enter your details to start chatting." },
  ]);
  ...
  const handleStartChat = (e: React.FormEvent) => {
    ...
    if (validate()) {
      ...
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          { role: "bot", text: `Namaste, ${userData.name.split(' ')[0]}! I have access to our full website knowledge — courses, lab setups, certificates, team, and contact info.\n\nWhat would you like to know? Tap a suggestion below or type your question.` }
        ]);
        setIsTyping(false);
      }, 1000);
    }
  ...
  const handleSend = async (e?: React.FormEvent, preset?: string) => {
    ...
    try {
      ...
      const replyText =
        data.text ||
        "I couldn't generate a response. Try asking about courses, lab setup, or contact details.";
  ...
  // Responses are handled by /api/chat with live website knowledge.
              ...
              <div className="flex items-center justify-between relative z-10">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      <span className="text-[9px] font-bold text-slate-400">Website-trained assistant</span>
                    </div>
                  </div>
                </div>
                <button 
                    ...
                    <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[88%] flex gap-3 ${m.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
                        ...
                        <div className={`p-4 rounded-[1.5rem] text-[12.5px] font-medium leading-relaxed shadow-[0_2px_10px_rgba(0,0,0,0.02)] ${
                          ...
                          {m.text.split('\n').map((line, idx) => (
                            <p key={idx} className={idx > 0 ? "mt-2" : ""}>{renderMessageText(line)}</p>
                          ))}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4 px-1 sm:px-2">
                    {QUICK_PROMPTS.map((chip) => (
                      <button 
                        ...
                        disabled={isTyping}
                        className="text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border border-slate-100 bg-slate-50 text-slate-500 hover:bg-[#C1121F] hover:text-white hover:border-[#C1121F] transition-all disabled:opacity-50"
                      >
                        {chip}
                      </button>
                    ))}
