            ...
            {clientReviews
              ...
              .map((testimonial, index) => (
                <div
                  ...
                  className="w-80 flex-shrink-0 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"
                >
                  <div className="mb-4 flex gap-0.5" aria-label={`${testimonial.rating} out of 5 stars`}>
                    {Array.from({ length: 5 }, (_, i) => (
                      <svg
                        key={i}
                        className={`h-4 w-4 ${i < testimonial.rating ? "text-amber-400" : "text-slate-200"}`}
                        viewBox="0 0 20 20"
                        fill="currentColor"
                        aria-hidden
                      >
                  ...
                  <p className="mb-4 text-sm leading-relaxed text-slate-600 line-clamp-5">
