...
const ApplyCertificateForm = ({ courses }: Props) => {
  ...
  // When a course is selected, auto-fill courseType and trainingHours
  const handleCourseChange = (e: ChangeEvent<HTMLSelectElement>) => {
    const selectedTitle = e.target.value;
    const course = courses.find((c) => c.title === selectedTitle);
    if (course) {
      setFormData((prev) => ({
        ...prev,
        courseName: course.title,
        courseType: course.isFree ? 'free' : 'paid',
        trainingHours: course.hours ? String(course.hours) : prev.trainingHours,
      }));
    } else {
  ...
  const handleNextStep = (e: FormEvent) => {
    ...
    if (!studentImage) { setSubmitMessage('Please upload your student image.'); return; }
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setSubmitMessage('');
    if (!studentImage) { setSubmitMessage('Please upload a student image.'); return; }
        ...
        <div className="container mx-auto max-w-4xl relative z-10">
          <div className="text-center mb-8">
            ...
            <div className="flex flex-wrap justify-center gap-3 mb-8">
              ...
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm px-4 py-2 rounded-full text-sm text-white border border-white/20">
                <svg className="w-4 h-4 text-yellow-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
      ...
      <section className="py-12 px-4 bg-gradient-to-br from-gray-50 to-blue-50">
        <div className="container mx-auto max-w-2xl">
            ...
            <div className="mb-6 max-w-2xl mx-auto px-4">
              <div className="flex items-center justify-center flex-wrap gap-2 sm:gap-0">
                <div className="flex items-center">
                  <div className={`flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-full font-bold text-sm ${step === 1 ? 'bg-[#C1121F] text-white ring-4 ring-[#C1121F]/20' : 'bg-green-500 text-white'}`}>
                    {step > 1 ? (
                  ...
                  <span className={`ml-2 font-semibold text-xs sm:text-sm md:text-base whitespace-nowrap ${step === 1 ? 'text-gray-800' : 'text-green-600'}`}>
                    Application Details
                  </span>
                </div>
                <div className={`w-12 sm:w-16 md:w-24 lg:w-32 h-1 mx-2 sm:mx-3 md:mx-4 rounded flex-shrink-0 ${step === 2 ? 'bg-[#C1121F]' : 'bg-gray-300'}`}></div>
                <div className="flex items-center">
                  <div className={`flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-full font-bold text-sm ${step === 2 ? 'bg-[#C1121F] text-white ring-4 ring-[#C1121F]/20' : 'bg-gray-300 text-gray-600'}`}>
                    2
                  </div>
                  <span className={`ml-2 font-semibold text-xs sm:text-sm md:text-base whitespace-nowrap ${step === 2 ? 'text-gray-800' : 'text-gray-500'}`}>
                    Payment
                  </span>
                </div>
              </div>
          ...
          <div className="max-w-2xl mx-auto bg-white p-6 md:p-8 rounded-xl shadow-lg border border-gray-100">
                ...
                <div className="mb-4">
                  ...
                  {formData.courseName && (
                    <p className="mt-1.5 text-xs text-blue-600 font-medium">
                      ✓ Selected: <span className="font-semibold">{formData.courseName}</span>
                    </p>
                  )}
                </div>

                {/* Course Type (auto-filled but still editable) */}
                <div className="mb-4">
                  ...
                  <div className="relative">
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        <span className="text-sm text-gray-500">{studentImage ? studentImage.name : 'Choose a file'}</span>
                      </div>
                      <span className="text-xs font-bold text-white bg-blue-600 px-3 py-1.5 rounded-md hover:bg-blue-700 transition-colors">Browse</span>
                    </label>
                    <input
                      type="file"
                      name="studentImage"
                ...
                {formData.courseName && (
                  <div className="mb-5 p-3 bg-blue-50 border border-blue-200 rounded-lg text-center">
                    <p className="text-sm text-blue-800 font-medium">
                      Course: <span className="font-bold">{formData.courseName}</span>
                    </p>
                  </div>
                )}
                ...
                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 mb-6 border border-blue-100 shadow-sm">
                  <div className="flex flex-col items-center">
                    <div className="bg-white p-3 rounded-lg shadow-md mb-4">
                      <Image src="/BankQR.png" alt="Nepatronix Payment QR Code" width={180} height={180} className="rounded-lg" />
                    </div>
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            {!paymentScreenshot && <span className="text-xs text-gray-400">PNG, JPG or JPEG (Max 5MB)</span>}
                          </div>
                        </div>
                        <span className="text-xs font-bold text-white bg-blue-600 px-3 py-1.5 rounded-md hover:bg-blue-700 transition-colors">Browse</span>
                      </label>
            ...
            {submitMessage && (
              <div className={`mt-6 p-4 rounded-lg text-center font-semibold ${submitMessage.includes('success') ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>
                {submitMessage}
              </div>
            )}
          </div>
